...
var aidRaster = {
  ...
  */
  ...
  calcStickOutLine: function (p1, p2) {
    ...
    var p1c = new paper.Point(this.shortenLine(p1.x, p1.y, p2.x, p2.y, this.pathExtendedWidth / 2));
    var p2c = new paper.Point(this.shortenLine(p2.x, p2.y, p1.x, p1.y, this.pathExtendedWidth / 2));


    var topPoints = this.expandLine(p1c.x, p1c.y, p2c.x, p2c.y, this.pathExtendedWidth / 2);
    var p1r = new paper.Point(topPoints[1]);
    var p1l = new paper.Point(topPoints[0]);


    var bottomPoints = this.expandLine(p2c.x, p2c.y, p1c.x, p1c.y, this.pathExtendedWidth / 2);
  ...
  calcStickPosition: function (prefs) {
    ...
    if (prefs.clockwise) {
      start = new paper.Point(this.getGridPoint(prefs.x), this.getGridPoint(prefs.y));
      end = new paper.Point(this.getGridPoint(prefs.x + prefs.width), this.getGridPoint(prefs.y + prefs.height));
    } else {
      start = new paper.Point(this.getGridPoint(prefs.x + prefs.width), this.getGridPoint(prefs.y));
      end = new paper.Point(this.getGridPoint(prefs.x), this.getGridPoint(prefs.y + prefs.height));
    }
  ...
  getGridPoint: function (value) {
    return ((value + 1) * (this.xSpaceWidth + this.xDotWidth)) - (this.xDotWidth / 2)
  },
...
function runLogo() {
  ...
  // full size bg
  var rectangle = new paper.Rectangle(new Point(0, 0), new Point(aidRaster.width, aidRaster.width));
